...
def forming_magic_square(s):
    ...
    for cic in changeable_idxs_combination:
        ...
        for nvp in nv_permutations:
            ...
            unique_group_by_changeable_idx_sum = [Counter([
                    sum([
                        permutations_by_changeable_idx[ug_idx]
                        if ug_idx in permutations_by_changeable_idx
                        else s_flat[ug_idx]
                        for ug_idx in unique_group_ci])
                    for unique_group_ci in unique_groups_ci]).most_common(1)
                for unique_groups_ci in groups_by_cic]

            ugbcis = [
                (idx, val[0])
                for idx, val in enumerate(unique_group_by_changeable_idx_sum)
                if val[0][1] == len(groups_by_cic[idx])]
            
            # print('groups_by_cic', groups_by_cic)
            if (not len(ugbcis)):
                continue
            ...
            unique_groups_15 = [
                val
                for val in unique_groups_sums
                if val == unique_groups_sums[0]
            ]
            unique_groups_total_sum = sum(unique_groups_sums)
            unique_groups_total_ave = (
                unique_groups_total_sum / len(unique_groups_sums))

            print('unique_groups_15', unique_groups_15)

            if len(unique_groups_15) != len(unique_groups):
                continue
